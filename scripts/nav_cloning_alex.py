...
class Net(nn.Module):
    def __init__(self, n_out):
        ...
        alexnet.fc = nn.Linear(4096, n_out)
        self.deconv1 = nn.ConvTranspose2d(1, 1, 11, stride=4, bias=False)  #padding=(2,2)
        self.deconv2 = nn.ConvTranspose2d(1, 1, 5, stride=1, bias=False)   #padding=(2,2)
        self.deconv3 = nn.ConvTranspose2d(1, 1, 3, stride=1, bias=False)   #padding=(1,1)
        self.deconv4 = nn.ConvTranspose2d(1, 1, 3, stride=1, bias=False)   #padding=(1,1)
        self.deconv5 = nn.ConvTranspose2d(1, 1, 3, stride=1, bias=False)   #padding=(1,1)
    ...
    #<Weight set>
        ...
        torch.nn.init.ones_(self.deconv5.weight)
    
    #<CNN layer>   
        self.cnn_layer = alexnet
...
class deep_learning:
    def __init__(self, n_action=1):
        #<tensor device choiece>
        self.device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
        torch.manual_seed(0)
        self.net = Net(n_action)
