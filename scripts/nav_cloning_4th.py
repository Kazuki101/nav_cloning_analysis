...
class Net(nn.Module):
    def __init__(self, n_channel, n_out):
        super().__init__()
    #<Network CNN 3 + FC 2> 
        self.conv1 = nn.Conv2d(n_channel, 16,kernel_size=4, stride=2)
        ...
        self.deconv1 = nn.ConvTranspose2d(1, 1, 4, stride=2, bias=False)
        self.deconv2 = nn.ConvTranspose2d(1, 1, 3, stride=2, bias=False)
        self.deconv3 = nn.ConvTranspose2d(1, 1, 3, stride=2, bias=False)
        self.deconv4 = nn.ConvTranspose2d(1, 1, 3, stride=1, bias=False)
    ...
    #<Weight set>
        ...
        torch.nn.init.ones_(self.deconv4.weight)
    ...
    def feature2image(self, conv1, conv2, conv3, conv4):
        ave1_reshape = torch.reshape(conv1, (1, 1, 23, 31))
        ave2_reshape = torch.reshape(conv2, (1, 1, 11, 15))
        ave3_reshape = torch.reshape(conv3, (1, 1, 5, 7))
        ave4_reshape = torch.reshape(conv4, (1, 1, 3, 5))
        image = self.deconv4(ave4_reshape) * ave3_reshape
...
class deep_learning:
    def __init__(self, n_channel=3, n_action=1):
        ...
        self.extractor = create_feature_extractor(self.net, ["relu", "relu_1", "relu_2", "relu_3"])
    ...
    def fv(self, img):
        ...
        conv4 = features["relu_3"]
        feature4 = conv4.to('cpu').detach().numpy().copy()
        ave4 = np.average(feature4[0],axis=0)
        ave4_ten = torch.from_numpy(ave4)
        ave4_ten = ave4_ten.cuda()
        fv_img = self.net.feature2image(ave1_ten, ave2_ten, ave3_ten, ave4_ten)
