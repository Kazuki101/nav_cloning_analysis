...
class deep_learning:
    ...
    def fv(self, img):
        ...
        self.net(x_ten)
        conv1 = FM_visualize(self.net.cnn_layer, 0) 
        feature1 = conv1.features
        ave1 = np.average(feature1[0],axis=0)
        ave1_ten = torch.from_numpy(ave1)
        ave1_ten = self.ave1_ten.cuda()
        conv2 = FM_visualize(self.net.cnn_layer, 2) 
        feature2 = conv2.features
        ave2 = np.average(feature2[0],axis=0)
        ave2_ten = torch.from_numpy(ave2)
        ave2_ten = self.ave2_ten.cuda()
        conv3 = FM_visualize(self.net.cnn_layer, 4) 
        feature3 = conv3.features
        ave3 = np.average(ave3[0],axis=0)
        ave3_ten = torch.from_numpy(feature3)
        ave3_ten = self.ave3_ten.cuda()
